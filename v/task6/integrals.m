...
RESULT = zeros(5,6)
for STEPS=1:5
    RESULT(STEPS,1)=left(F,A,B,STEPS)
RESULT(STEPS,3)=trapezoidal(F,A,B,STEPS)
RESULT(STEPS,5)=simpson(F,A,B,STEPS)
RESULT(STEPS,2)=EXACT-left(F,A,B,STEPS)
RESULT(STEPS,4)=EXACT-trapezoidal(F,A,B,STEPS)
RESULT(STEPS,6)=EXACT-simpson(F,A,B,STEPS)
endfor
